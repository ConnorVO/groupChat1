//
//  ChatSegmentedControlCell.swift
//  GroupChat1
//
//  Created by Connor Van Ooyen on 11/7/17.
//  Copyright © 2017 Connor Van Ooyen. All rights reserved.
//

import UIKit

class ChatSegmentedControlCell: UICollectionViewCell {
    
    var onMessagesLabelTapped : (() -> Void)? = nil
    var onStarredLabelTapped : (() -> Void)? = nil
    ...
    lazy var secondButton: UIButton = {
        let btn = UIButton(type: .system)
        btn.setTitle("Starred", for: .normal)
        btn.titleLabel?.font = UIFont.systemFont(ofSize: 18)
    ...
    @objc func handleSecondBtn() {
        if let onStarredLabelTapped = self.onStarredLabelTapped {
            onStarredLabelTapped()
        }
